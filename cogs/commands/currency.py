...
class Currency(commands.Cog):
	...
	async def balance(self, ctx, user: discord.Member = None):
		...
		em.add_field(
			name="Pocket Money:",
			value=f"`{monetize(person.wallet)}`",
			inline=False
		)
		em.add_field(
			name="Bank Balance:",
			value=f"`{monetize(person.bank)}`",
			inline=False
		)
		em.set_footer(
			text=f"{richness(person.wallet, person.bank)}"
		)
	...
	async def rob(self, ctx, user: discord.Member):
		...
		if chances < 35:
			...
			em = discord.Embed(description=f"The neighbour's dog started barking at you and you tried to flee. But given how clumsy you are, you dropped your wallet. Classic {ctx.author.display_name} move.")
			await ctx.reply(embed=em, mention_author=False)
			...
			em = discord.Embed(description=f"You entered {user.display_name}'s house and tried to rob them but they beat the shit out of you and you had to pay them {amount} ⬢ in order to get away. Sucks to be you LOL")
			await ctx.reply(embed=em, mention_author=False)
